...
class FactualTravelInfoAnalyzerTest {

    private final FactualTravelInfoAnalyzer analyzer = new FactualTravelInfoAnalyzer();
    private final FactualTravelInfoConfig config = new FactualTravelInfoConfig("08:02", "09:15", 0);
    ...
    void detectsZeroChangeExpressions() {
        ...
        for (String expression : expressions) {
            FactualTravelInfoExtraction extraction = analyzer.extract(
                    1,
                    "Abfahrt 08:02, Ankunft 09:15, " + expression + ".",
                    config
            );

            assertThat(extraction.status()).as(expression).isEqualTo(FactualTravelInfoStatus.SUCCESS);
    ...
    void acceptsObservedGermanResponses() {
        ...
        for (int i = 0; i < responses.size(); i++) {
            FactualTravelInfoExtraction extraction = analyzer.extract(i + 1, responses.get(i), config);

            assertThat(extraction.status()).as(responses.get(i)).isEqualTo(FactualTravelInfoStatus.SUCCESS);
    ...
    void marksMissingFactsAsOutlierAndKeepsExtraTimes() {
        ...
        assertThat(extraction.status()).isEqualTo(FactualTravelInfoStatus.OUTLIER);
